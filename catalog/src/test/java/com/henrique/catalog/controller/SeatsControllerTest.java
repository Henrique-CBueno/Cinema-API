...
class SeatsControllerTest {
        ...
        class GetAllSeatsByCinemaRoom {
                ...
                void shouldGetAllSeatsWithHttpOK() {
                        ...
                        verify(seatsService, times(1)).getSeatsByCinemaRoom(UUID.fromString(roomId),
        ...
        @Nested
        class CheckSeatsExistence {

                @Test
                void shouldReturnOkWithResultBody() {
                        // Arrange
                        String cinemaId = UUID.randomUUID().toString();
                        String roomId = UUID.randomUUID().toString();
                        List<UUID> seatIds = List.of(UUID.randomUUID(), UUID.randomUUID());
                        SeatsExistenceResDTO responseDto = new SeatsExistenceResDTO(true, List.of());

                        doReturn(responseDto)
                                        .when(seatsService)
                                        .validateSeatsInRoom(UUID.fromString(cinemaId), UUID.fromString(roomId),
                                                        seatIds);

                        // Act
                        ResponseEntity<SeatsExistenceResDTO> response = seatsController.checkSeatsExistence(
                                        cinemaId,
                                        roomId,
                                        seatIds);
                        ...
                        assertNotNull(response.getBody());
                        assertTrue(response.getBody().allExists());
                        assertTrue(response.getBody().missingSeatIds().isEmpty());
                }
                ...
                void shouldPassCorrectParametersToService() {
                        ...
                        ArgumentCaptor<UUID> cinemaCaptor = ArgumentCaptor.forClass(UUID.class);
                        ArgumentCaptor<UUID> roomCaptor = ArgumentCaptor.forClass(UUID.class);
                        ArgumentCaptor<List> seatIdsCaptor = ArgumentCaptor.forClass(List.class);

                        doReturn(new SeatsExistenceResDTO(true, List.of()))
                                        .when(seatsService)
                                        .validateSeatsInRoom(cinemaCaptor.capture(), roomCaptor.capture(),
                                                        seatIdsCaptor.capture());

                        // Act
                        seatsController.checkSeatsExistence(cinemaId.toString(), roomId.toString(), seatIds);

                        // Assert
                        assertEquals(cinemaId, cinemaCaptor.getValue());
                        assertEquals(roomId, roomCaptor.getValue());
                        assertEquals(seatIds, seatIdsCaptor.getValue());
                }
        }
}
